...
def serialize_incident(incident: Incident) -> dict:
    return {
        ...
        "time": format_relative_time(incident.occurred_at),
        "distance": incident.distance_label or "N/A",
...
def load_incidents_from_db():
    with get_session() as session:
        rows = session.query(Incident).order_by(Incident.occurred_at.desc()).all()
        return [serialize_incident(row) for row in rows]


def load_reports_from_db():
    with get_session() as session:
        rows = session.query(Report).order_by(Report.created_at.desc()).all()
...
def refresh_session_data(section: str = "all"):
    if section in ("all", "incidents"):
        st.session_state.incidents = load_incidents_from_db()
    if section in ("all", "reports"):
        st.session_state.reports = load_reports_from_db()
    if section in ("all", "notifications"):
        st.session_state.notifications = load_notifications_from_db()


def create_report(incident_type: str, description: str, location: str):
    with get_session() as session:
        new_report = Report(
            type=incident_type,
...
def create_notification(title: str, description: str, unread: bool = True):
    with get_session() as session:
...
def mark_all_notifications_read():
    with get_session() as session:
        session.execute(update(Notification).values(unread=False))
        session.commit()
    ...
    with col2:
        if st.button("🚨 EMERGENCY", key="emergency_btn", help="Click in case of emergency"):
            ...
            create_notification(
                ...
                description='Emergency services have been contacted. Stay safe and follow instructions.',
                unread=True
            )
            refresh_session_data("notifications")
            st.rerun()
            ...
            with col2:
                if st.form_submit_button("✅ Submit Report"):
                    if incident_type and description:
                        create_report(incident_type, description, location)
                        refresh_session_data("reports")
                        st.session_state.show_report_form = False
