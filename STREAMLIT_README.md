...
1. **Start PostgreSQL:**  
   ```bash
   docker run --name civicguardian-db \
     -e POSTGRES_PASSWORD=postgres \
     -e POSTGRES_DB=civicguardian \
     -p 5432:5432 \
     -d postgres:15
   ```
2. **Set the connection string (before running Streamlit):**
   - **PowerShell**
     ```powershell
   ...
   - **macOS/Linux**
     ...
     ```
3. **Run the app.** Tables (`incidents`, `reports`, `notifications`) are created automatically and seeded with demo data on first launch.
