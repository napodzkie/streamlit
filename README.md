...
### PostgreSQL Setup for Streamlit App

The Streamlit experience now persists data using PostgreSQL via SQLAlchemy.

1. **Start a local PostgreSQL instance** (for example with Docker):
   ```bash
   docker run --name civicguardian-db -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=civicguardian -p 5432:5432 -d postgres:15
   ```
2. **Set the connection string** before launching Streamlit:
   ```bash
   # Windows PowerShell
   $Env:DATABASE_URL = "postgresql+psycopg2://postgres:postgres@localhost:5432/civicguardian"
   # macOS/Linux
   export DATABASE_URL="postgresql+psycopg2://postgres:postgres@localhost:5432/civicguardian"
   ```
3. **Run the Streamlit app** (tables and demo data seed automatically):
   ```bash
